...
export default {
  ...
  methods: {
    onTreeNodeSelect(id) {
      ...
      this.$refs.datarule.show(this.selectedKeys[0], this.versionId)
    },
    ...
    handleSubmit(exit) {
      ...
      axios
        .post('/system/recruitFunctionVersionMenu/add', {
          versionId: that.versionId,
          permissionIds: that.checkedKeys,
          halfCheckedKeys: that.halfCheckedKeys,
        })
        .then((res) => {
          if (res.success) {
    ...
    loadData() {
      queryTreeListForRole().then((res) => {
        ...
        // queryRolePermission({ versionId: this.versionId }).then((res) => {
        //   this.checkedKeys = [...res.result]
        //   this.defaultCheckedKeys = [...res.result]
        //   this.expandedKeysss = this.allTreeKeys
        //   console.log(this.defaultCheckedKeys)
        // })
        axios.get('/system/recruitFunctionVersionMenu/queryByVersionId?versionId='+this.versionId).then((res)=>{
          this.checkedKeys = [...res.result]
          this.defaultCheckedKeys = [...res.result]
          this.expandedKeysss = this.allTreeKeys
          console.log(this.defaultCheckedKeys)
