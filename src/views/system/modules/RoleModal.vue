...
<script>
  ...
  export default {
    ...
    methods: {
      initRoleCode () {
        let that = this
         getAction('/sys/role/getTenantRoleCodeByTenantId').then((res) => {
             if (res.success) {
                console.log('获取到的角色编码信息为：' + res.result)
                that.model.roleCode = res.result.roleCode
                this.roleDisabled = res.result.isTenant
             }
         })
      },
      add () {
